...
public class FileTailAppender {
	...
	public static void main(String[] args) {
		...
		try {
			...
			while ((singleLine = reader.readLine()) != null) { // 읽을게 없으면 null 리턴
				txt.append(singleLine).append("\n");
				if (singleLine.length() <= 0 || isCommentLine(singleLine)) {
					continue;
				}
				targetList.add(singleLine);
			}
			logger.debug("TARGET_LOCATION_FILE 내용:{}{}", "\n", txt);
	...
	private static void appendBlank(String targetLocation) throws IOException {
		...
		Writer output = new BufferedWriter(new FileWriter(target, true));
		output.append(" ");
